...
function GetHelpTextForUnit(iUnitID, bIncludeRequirementsInfo)
	...
	-- Maintenance
	local iMaintenance = pUnitInfo.ExtraMaintenanceCost*0.5;
...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	if (iFood ~= nil and iFood ~= 0) then
		table.insert(lines, Locale.ConvertTextKey("TXT_KEY_EOM_BUILDING_FOOD_MOD", iFood));
	end
	...
	-- Required building(s)
	local condition = "BuildingType = '" .. pBuildingInfo.Type .. "'";
	for row in GameInfo.Building_ClassesNeededInCity( condition ) do
		...
		if(buildingClass) then
			...
			local condition2 = "CivilizationType = '" .. GameInfo.Civilizations[pActivePlayer:GetCivilizationType()].Type .. "' AND BuildingClassType = '" .. buildingClass.Type .. "'";
			for row in GameInfo.Civilization_BuildingClassOverrides( condition2 ) do
				local building = GameInfo.Buildings[row.BuildingType];
				if(building) then
					thisBuildingInfo = building
				end
			end
			if thisBuildingInfo then
				table.insert(lines, "[ICON_BULLET] Required: [COLOR_POSITIVE_TEXT]" .. Locale.ConvertTextKey( thisBuildingInfo.Description ) .. "[ENDCOLOR]")
			end
		end
	end

	-- Fresh Water
	--if (pBuildingInfo.FreshWater) then
	--	table.insert(lines, "[ICON_BULLET] City must have access to fresh water.")
	--end
