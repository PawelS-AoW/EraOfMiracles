...
function InitialSetup()
	...
	-- add the pipes EOM START (ALZARA FIX to pipes)
	...
	for row in GameInfo.Technology_PrereqTechs() do
		...
		local show_line = row.EoM_ShowLine;
		-- EoM change - show only lines with EoM_ShowLine set to 1
		if tech and prereq and show_line == 1 then
	...
	for row in GameInfo.Technology_PrereqTechs() do
		...
		if tech and prereq and show_line == 1 then
			...
			-- EOM START - make vertical pipes appear regardless of the vertical distance

			local dist = tech.GridY - prereq.GridY;
			...
			if dist >= 1 then
				...
				local size = { x = 32; y = (blockSpacingY * (dist * 2 - 1) / 2) + 8; };
