...
-- modified by Pawe³ Strzelec (2012-2013)
...
function MapConstants:New()
	...
	mconst.atollNorthLatitudeLimit = 50
	mconst.atollSouthLatitudeLimit = -50
	...
	mconst.southAttenuationRange = 0.0
...
function GenerateTempMaps(elevationMap)
	...
	local bottomTempLat = mc.bottomLatitude + zenith
	for y = 0,elevationMap.height - 1,1 do
		for x = 0,elevationMap.width - 1,1 do
			local i = summerMap:GetIndex(x,y)
			local lat = summerMap:GetLatitudeForY(y)
			local latPercent = lat / 90
			local temp = (math.cos(latPercent * math.pi * 0.5) * 0.5) + 0.5
			if elevationMap:IsBelowSeaLevel(x,y) then
	...
	for y = 0,elevationMap.height - 1,1 do
		for x = 0,elevationMap.width - 1,1 do
			...
			temperatureMap.data[i] = (winterMap.data[i] + summerMap.data[i]) * (1.0 - aboveSeaLevelMap.data[i]/2) -- height factor on temperature
		end
	end
		...
		if not (topY == -1 and bottomY == -1) then
			...
			for y = yStart,yStop ,incY do
				...
				for x = xxStart,xxStop - incX,incX do
					local i = elevationMap:GetIndex(x,y)
					if i>=0 then sortedGeoMap[geoIndex] = {x,y,geoMap.data[i]}
					geoIndex = geoIndex + 1 end
				end
			end
		end
	end
...
function PlacePossibleOasis(x,y)
	...
	if not plot:IsHills() and not plot:IsMountain() and plot:GetTerrainType() == terrainDesert then
		...
		if canPlace and PWRandint (0,2) == 0 then
			plot:SetFeatureType(featureOasis,-1) -- 1/3 chance for Oasis if good place found
		end
	end
end
	...
	if plot:GetTerrainType() == shallowWater then
		...
		if latitude < mc.atollNorthLatitudeLimit and latitude > mc.atollSouthLatitudeLimit then
			...
			if deepCount >= mc.atollMinDeepWaterNeighbors then
				if PWRandint (0,5) == 0 then plot:SetFeatureType (featureAtoll,-1) end
			end
		end
	end
end
...
function GetMapScriptInfo()
	...
	return {
		Name = "*Perfect Flat World*",
		Description = "Modified version of the Perfect World 3 map script by Cephalo, designed for Chronicles of Adea.",
		IsAdvancedMap = 0,
...
function GetMapInitData(worldSize)
	local worldsizes = {
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = {48, 30},
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = {56, 35},
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = {64, 40},
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = {80, 50},
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = {96, 60},
...
function GeneratePlotTypes()
	...
	elevationMap = GenerateElevationMap(gridWidth,gridHeight,false,false)
	--FillInLakes()
	...
	local diffMap = FloatMap:New(gridWidth,gridHeight,false,false)
...
function AddFeatures()
	print("Adding Features Perfect Flat World");
	...
	local allTreesThreshold = rainfallMap:FindThresholdFromPercent (mc.allTreesPercent,false,true)
	...
	for y = 0 , gridHeight - 1, 1 do
		for x = 0 , gridWidth - 1, 1 do			
			...
			if not (plot:IsWater () or plot:IsMountain ()) then
				if rainfallMap.data [i] >= marshThreshold and plot:CanHaveFeature (featureMarsh) then
					plot:SetFeatureType (featureMarsh,-1)
				elseif rainfallMap.data [i] >= jungleThreshold and temperatureMap.data[i] >= mc.jungleMinTemperature then
					plot:SetFeatureType (featureJungle,-1)
				elseif rainfallMap.data [i] >= zeroTreesThreshold and temperatureMap.data[i] >= mc.treesMinTemperature then
					local ForestChance = 2 -- 2/3 Plains, Tundra
					if rainfallMap.data [i] >= allTreesThreshold then ForestChance = 3
					elseif plot:GetTerrainType () == TerrainTypes.TERRAIN_GRASS then ForestChance = 1 end -- 1/3 Grass
					if PWRandint (0,2) < ForestChance then
						plot:SetFeatureType (featureForest,-1)
					end
				end
				if plot:CanHaveFeature(featureFloodPlains) and PWRandint (0,2) > 0 then -- 2/3 chance for Flood Plains
...
function DetermineContinents ()
	print("Determining continents for art purposes (Perfect Flat World)")

 	-- adjust terrain under jungles to 2/3 grassland, 1/3 plains

	for i, plot in Plots () do
		if plot:GetFeatureType () == GameInfoTypes.FEATURE_JUNGLE then
			if PWRandint (0,2) == 0 then plot:SetTerrainType (TerrainTypes.TERRAIN_PLAINS,false,true)
			else plot:SetTerrainType (TerrainTypes.TERRAIN_GRASS,false,true) end
		end
	end
	...
	for i, plot in Plots () do
		...
		local feature = plot:GetFeatureType ()
 		if not plot:IsWater () then -- water has always continent type 0
 			if plot:IsMountain () then
				if terrain == GameInfoTypes.TERRAIN_SNOW or terrain == GameInfoTypes.TERRAIN_TUNDRA then plot:SetContinentArtType (4) -- snow/tundra mnts -> Europe
				elseif terrain == GameInfoTypes.TERRAIN_GRASS then plot:SetContinentArtType (1) -- grass mnts -> America
				elseif terrain == GameInfoTypes.TERRAIN_PLAINS then plot:SetContinentArtType (2) -- plains mnts -> Asia
				else plot:SetContinentArtType (3) end -- desert mnts -> Africa
			elseif feature == GameInfoTypes.FEATURE_FOREST then
				if terrain == GameInfoTypes.TERRAIN_GRASS then plot:SetContinentArtType (4) -- forest on grass -> Europe
				elseif terrain == GameInfoTypes.TERRAIN_PLAINS then plot:SetContinentArtType (1) -- forest on plains -> America
				else plot:SetContinentArtType (PWRandint (2,3)) end -- forest on tundra -> Africa/Asia
			else
				local ct = plot:GetContinentArtType ()
				if PWRandint (0,2) == 0 then ct = PWRandint (1,4) end -- add some mix to continent types
				if terrain == GameInfoTypes.TERRAIN_GRASS and ct == 3 then ct = 4 end -- Arfican grass looks almost like Plains, change to European which looks best
				plot:SetContinentArtType (ct)
			end
		end
	end
end
